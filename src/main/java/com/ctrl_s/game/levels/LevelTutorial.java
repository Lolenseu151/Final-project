...
/**
 * Tutorial map for the Archive floor. Simple layout with a highlighted light
 * area
...
public class LevelTutorial implements Level, BackgroundedLevel {
    ...
    // centralized shredder visual (assigned by LevelManager as the shared animated
    // shredder)
    ...
    // Should the tutorial overlay box be shown? Allows the player to dismiss it
    // with OK.
    ...
    // Optional explicit positions for the documents that appear after the final
    // caption.
    // Edit this array to specify exact X/Y pixel positions for each spawned
    // document.
    // Example:
    // public static float[][] TUTORIAL_DOC_POSITIONS = new float[][] { {400f,300f},
    // {480f,300f}, {560f,300f}, {640f,300f} };
    ...
    // Allow this level to request a different document draw size (only affects
    // visuals)
    ...
    public float getDocumentSize() {
        return DOC_SIZE * documentScale;
    }

    public void setDocumentScale(float scale) {
        if (scale > 0f)
    ...
    public String getBackgroundPath() {
        ...
        return "assets/maps/tutorial_map.png";
    }
    ...
    public void renderBackground(SpriteBatch batch, com.badlogic.gdx.graphics.Texture backgroundTex) {
        // Draw the default background via LevelManager, then overlay a subtle light at
        // propertyFilesLight
        ...
        try {
            Gdx.app.log("LevelTutorial",
                    "renderBackground called; finalOverlayVisible=" + finalOverlayVisible + " shredder=" + shredder);
        } catch (Exception ignored) {
        }
        // the actual light highlight will be drawn by the LevelManager or GameScreen
        // overlay logic
        // Draw the final tutorial overlay image full-screen when requested (or forced
        // by debug)
        ...
        if (effectiveFinalOverlayVisible) {
            if (finalTalkingTex == null) {
                try {
                    finalTalkingTex = new com.badlogic.gdx.graphics.Texture(
            ...
            // Draw the overlay text above the image (centered near the top) with typing
            // transition
            String fullFinalText = (finalOverlayStage == 0) ? FINAL_OVERLAY_FIRST_TEXT : FINAL_OVERLAY_SECOND_TEXT;
            try {
                if (finalOverlayFont == null) {
                    // Try to load smallwhite font (match GameScreen behavior), fallback to default
                    try {
                ...
                if (finalOverlayFont != null) {
                    ...
                    gl.setText(finalOverlayFont, visibleText, com.badlogic.gdx.graphics.Color.WHITE, wrapWidth,
                    ...
                    boolean hovered = (mx >= btnX && mx <= btnX + glBtn.width && my >= btnY - glBtn.height
                            && my <= btnY);
    ...
    public Rectangle getPropertyFilesLight() {
        return propertyFilesLight;
    }
    ...
    public boolean isTalkingOverlayVisible() {
        return talkingOverlayVisible;
    }

    public void setTalkingOverlayVisible(boolean v) {
        this.talkingOverlayVisible = v;
    }
    ...
    // If true, the final overlay has been dismissed and should not re-open
    // automatically
    private boolean finalOverlayConsumed = false;

    public boolean isFinalOverlayVisible() {
        return finalOverlayVisible;
    }
    ...
    public boolean isFinalOverlayConsumed() {
        return finalOverlayConsumed;
    }

    // Tutorial text getters/setters so the game or designer can customize the
    // overlay
    ...
    public void setTutorialHint(String hint) {
