...
public class Level3_1 implements Level, BackgroundedLevel {
    ...
    // declare expected docs for this continuation area (must match actual added
    // docs)
    ...
    public void init() {
        ...
        // LEFT BOUNDARY WALL - prevents player from going too far left (allow some
        // space for return transition)
        ...
        // documents (keep same count as Level3) - randomly scattered but always on flat
        // platforms
         ...
         * float slopeRightX = slopeLeftX + (float) (Math.cos(slopeAngleRad) *
         * slopeLength);
         * float slopeRightY = slopeLeftY + (float) (Math.sin(slopeAngleRad) *
         * slopeLength);
         * sloped.add(new SlopedPlatform(slopeLeftX, slopeLeftY, slopeRightX,
         * slopeRightY));
         */
        ...
        try {
            Level3.SHARED_TOTAL_DOCS = documents.size + Level3.DECLARED_DOCS;
    ...
    public int getTotalDocuments() {
        ...
        try {
            return Level3.SHARED_TOTAL_DOCS;
        } catch (Throwable ignored) {
            return totalDocs;
        }
    }
    ...
    public String getBackgroundPath() {
        return "assets/maps/level_3_1_map.png";
    }
    ...
    public void updateBackground(float deltaTime, ILevelManager levelMgr,
        ...
        try {
            if (player != null && player.getBounds() != null && platforms.size > 0) {
                ...
                try {
                    movingLeft = (player.getVelocity() != null && player.getVelocity().x < -40f);
                } catch (Exception ignored) {
                    movingLeft = false;
                }
    ...
    public void renderOverlay(SpriteBatch batch) {
        if (!DRAW_PLATFORM_OVERLAY || batch == null)
            return;
        try {
            if (platformPixel == null) {
