...
public class Level5_1 implements Level, BackgroundedLevel {
    ...
    // Shredder position/size constants removed as unused
    ...
    private static final String BG_CONTINUATION = "assets/maps/level_5_1_map.png";
    ...
    public void init() {
        ...
        try {
            SHARED_TOTAL_DOCS = documents.size + Level5_1.DECLARED_DOCS;
    ...
    @Override
    public void dispose() {
        /* per-level resources disposed by LevelManager */ }
    ...
    public void updateBackground(float deltaTime, ILevelManager levelMgr,
        ...
        try {
            if (player != null && player.getBounds() != null && platforms.size > 0 && transitionCooldown <= 0) {
                ...
                try {
                    movingLeft = (player.getVelocity() != null && player.getVelocity().x < 0);
                } catch (Exception ignored) {
                    movingLeft = false;
                }
