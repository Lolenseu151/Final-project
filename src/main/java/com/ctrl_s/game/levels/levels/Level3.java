...
import com.ctrl_s.game.levels.ILevelManager; // fallback legacy manager
...
public class Level3 implements Level, BackgroundedLevel {
    ...
    private final com.badlogic.gdx.utils.Array<SlopedPlatform> sloped = new com.badlogic.gdx.utils.Array<>();
    ...
    // Toggle for debugging: when false, platforms remain invisible like other
    // levels
    ...
    private static final String BG_FIRST = "assets/maps/level_3_map.png";
    ...
    public void init() {
        ...
        // platforms.add(new Rectangle(1250, 603, 20, 170)); // Right wall for roof
        // section
        ...
        sloped.add(new SlopedPlatform(leftX, leftY, rightX, rightY));
        ...
        try {
            SHARED_TOTAL_DOCS = documents.size + Level3_1.DECLARED_DOCS;
    ...
    public int getTotalDocuments() {
        return SHARED_TOTAL_DOCS;
    }

    // Expose sloped platforms for managers that support slopes.
    public com.badlogic.gdx.utils.Array<SlopedPlatform> getSlopedPlatforms() {
        return sloped;
    }

    @Override
    public void dispose() {
        // Dispose per-level resources if any. Also dispose cached platformPixel to
        // avoid small leak.
        try {
            if (platformPixel != null) {
                try {
                    platformPixel.dispose();
                } catch (Exception ignored) {
                }
                platformPixel = null;
            }
    ...
    // Provide entrance spawn positions so GameScreen can set initial player
    // position
    public float[] getEntranceSpawn() {
        ...
        // Adjusted spawn point for better positioning when entering from level select
        // screen
    ...
    public void renderBackground(SpriteBatch batch, Texture backgroundTex) {
        ...
        // shared shredder visuals are handled by LevelManager (no per-level shredder
        // draw)
    }

    @Override
    public void renderOverlay(SpriteBatch batch) {
        if (!DRAW_PLATFORM_OVERLAY)
            return; // Keep platforms invisible unless debugging
        if (batch == null || switchedToContinuation)
            return;
        try {
            if (platformPixel == null) {
            ...
            for (Rectangle p : platforms) {
                try {
                    batch.draw(platformPixel, p.x, p.y, p.width, p.height);
                } catch (Exception ignored) {
                }
            }
