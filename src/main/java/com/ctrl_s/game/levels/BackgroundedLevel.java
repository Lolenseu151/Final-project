...
/**
 * Optional interface for levels that provide a background image with
 * interactive logic.
 * Backgrounds can respond to documents collected, obstacles hit, and player
 * state.
 */
public interface BackgroundedLevel {
    /**
     * Return the internal/relative path to the background image (e.g.
     * "Level1Map.png"),
    ...
    /**
     * Update background state based on level manager, documents, obstacles, and
     * player.
    ...
    /**
     * Optional: render overlays that must appear on top of level objects
     * (documents, platforms).
     * Called by LevelManager after documents and sprites are drawn so overlays
    ...
    /**
     * If true the level's overlay should block core gameplay updates (player
     * movement,
     * timers, collisions). Default is false. Levels that show blocking dialogs
     * should
     * return true while the dialog is visible.
     */
    default boolean isOverlayBlocking() {
        return false;
    }
}
