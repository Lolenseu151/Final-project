...
import com.ctrl_s.game.entities.LaserFX;
...
public class LevelManager2 implements ILevelManager {
    // make active instance discoverable by levels (so we don't have to pass
    // LevelManager2 into the old API)
    private static LevelManager2 ACTIVE = null;

    public static LevelManager2 getActive() {
        return ACTIVE;
    }
    ...
    public Level getCurrentLevelA() {
        return currentLevelA;
    }

    public Level getCurrentLevelB() {
    ...
    private LevelShredder sharedShredder;
    ...
    public LevelManager2() {
        ...
        // load document spritesheet
        boolean loaded = false;
        try {
            documentSheetTex = new Texture(Gdx.files.internal("assets/sprites/items/documents.png"));
            Gdx.app.log("LevelManager2", "Loaded document spritesheet: assets/sprites/items/documents.png");
            loaded = true;
        } catch (Exception e) {
            loaded = false;
        }
    ...
    private void placeShredder() {
        // This should not be called if level has shredder, but as fallback use default
        // position
        float screenWidth = Gdx.graphics.getWidth();
    ...
    public float update(float deltaTime, Player player) {
        ...
        // Pass 'this' (ILevelManager) to updateBackground so levels can use either
        // manager type
        try {
            if (currentLevelA instanceof BackgroundedLevel) {
                ((BackgroundedLevel) currentLevelA).updateBackground(deltaTime, this, this.documents, this.obstacles,
                        player);
            }
        } catch (Exception ignored) {
        }
        ...
        try {
            if (currentLevelB instanceof BackgroundedLevel) {
                ((BackgroundedLevel) currentLevelB).updateBackground(deltaTime, this, this.documents, this.obstacles,
                        player);
            }
        } catch (Exception ignored) {
        }
        ...
        for (int i = documents.size - 1; i >= 0; i--) {
            ...
            if (player.getBounds().overlaps(doc)) {
                ...
                try {
                    if (currentLevelA != null
                            && currentLevelA.getClass().getSimpleName().toLowerCase().contains("tutorial")
                            && documentsCollected == 1) {
                        try {
                            java.lang.reflect.Method m = currentLevelA.getClass().getMethod("setTalkingOverlayVisible",
                                    boolean.class);
                            m.invoke(currentLevelA, true);
                        } catch (Exception ignored) {
                        }
                    }
                    if (currentLevelB != null
                            && currentLevelB.getClass().getSimpleName().toLowerCase().contains("tutorial")
                            && documentsCollected == 1) {
                        try {
                            java.lang.reflect.Method m = currentLevelB.getClass().getMethod("setTalkingOverlayVisible",
                                    boolean.class);
        ...
        try {
            if (currentLevelA != null && currentLevelA.getClass().getSimpleName().toLowerCase().contains("tutorial")
                ...
                try {
                    ...
                    if (!vis && !cons) {
                        java.lang.reflect.Method setFinal = currentLevelA.getClass().getMethod("setFinalOverlayVisible",
                                boolean.class);
        ...
        try {
            if (currentLevelB != null && currentLevelB.getClass().getSimpleName().toLowerCase().contains("tutorial")
                    && documentsCollected >= 4) {
                try {
                    ...
                    if (!vis && !cons) {
                        java.lang.reflect.Method setFinal = currentLevelB.getClass().getMethod("setFinalOverlayVisible",
                                boolean.class);
        ...
        if (player != null && player.getBounds() != null && lasers != null && lasers.size > 0
                && laserHitCooldown <= 0f) {
            for (Rectangle laser : lasers) {
                if (laser != null && player.getBounds().overlaps(laser)) {
                    timePenalty += LASER_PENALTY_SECONDS;
                    try {
                        player.applySlow(LASER_SLOW_SECONDS);
                    } catch (Exception ignored) {
                    }
                    laserHitCooldown = LASER_COOLDOWN_SECONDS;
                    Gdx.app.log("LevelManager2",
                            "Player hit laser: -" + LASER_PENALTY_SECONDS + "s penalty and slowed");
                    break;
                }
            }
        }
    ...
    public void render(ShapeRenderer shapeRenderer, SpriteBatch batch, BitmapFont font) {
        ...
        for (Rectangle obstacle : obstacles) {
            ...
            try {
                if (JSObstacle.isRegisteredRect(obstacle))
                    continue;
            } catch (Exception ignored) {
            }
        ...
        try {
            // Prefer currentLevelB overlay (if present) so continuation overlays draw above
            // A
            if (currentLevelB instanceof BackgroundedLevel) {
                batch.begin();
    ...
    public void loadTwoMaps(Level levelA, Level levelB) {
        // Check if we're reloading the same level pair (e.g., Level3 + Level3_1
        // transition)
        // If so, preserve the document count to maintain continuity
        boolean isSameLevelPair = false;
        int preservedDocCount = 0;

        // Check for Level3/Level3_1 transitions (special case for single->dual map
        // transitions)
        if (currentLevelA != null && levelA != null) {
            ...
            if (hasLevel3 && hasLevel3_1) {
                ...
                Gdx.app.log("LevelManager2",
                        "Preserving document count (" + preservedDocCount + ") for Level3/Level3_1 transition");
            }
        }
        ...
        // For same level pair transitions, preserve the documents array (already
        // collected docs removed)
        ...
        if (isSameLevelPair && levelB != null) {
            ...
            // set documents collected based on combined total vs current list
            documentsCollected = (isSameLevelPair) ? preservedDocCount : 0;
        }
        ...
        try {
            if (levelA != null)
                platforms.addAll(levelA.getPlatforms());
        } catch (Exception ignored) {
        }
        try {
            if (levelB != null)
                platforms.addAll(levelB.getPlatforms());
        } catch (Exception ignored) {
        }
        ...
        try {
            if (levelA != null)
                obstacles.addAll(levelA.getObstacles());
        } catch (Exception ignored) {
        }
        try {
            if (levelB != null)
                obstacles.addAll(levelB.getObstacles());
        } catch (Exception ignored) {
        }

        // Merge sloped platforms
        slopedPlatforms.clear();
        try {
            if (levelA != null) {
                java.lang.reflect.Method m = levelA.getClass().getMethod("getSlopedPlatforms");
                com.badlogic.gdx.utils.Array<SlopedPlatform> s = (com.badlogic.gdx.utils.Array<SlopedPlatform>) m
                        .invoke(levelA);
                if (s != null)
                    slopedPlatforms.addAll(s);
            }
        } catch (Exception ignored) {
        }
        try {
        ...
        // Merge lasers and create effects
        lasers.clear();
        laserFXList.clear();

        // Helper to add lasers from level
        try {
            if (levelA != null) {
                Array<Rectangle> l = levelA.getLasers();
                if (l != null) {
                    lasers.addAll(l);
                    for (Rectangle r : l) {
                        try {
                            laserFXList.add(new LaserFX(r));
                        } catch (Exception ignored) {
                        }
                    }
                }
        ...
        try {
            if (levelB != null) {
                Array<Rectangle> l = levelB.getLasers();
                if (l != null) {
                    lasers.addAll(l);
                    for (Rectangle r : l) {
                        try {
        ...
        // Win condition: check BOTH map objects for a shredder (usually only one has
        // it)
        shredder = null;
        try {
            if (levelA != null && levelA.getShredder() != null)
                shredder = levelA.getShredder();
        } catch (Exception ignored) {
        }
        try {
            if (levelB != null && levelB.getShredder() != null)
                shredder = levelB.getShredder();
        } catch (Exception ignored) {
        }

        // Initialize combined background
        // Prefer B background if present (continuation), else A
        backgroundTex = null; // reset
        try {
            String path = null;
            if (levelB != null)
                path = levelB.getBackgroundPath();
            if (path == null && levelA != null)
                path = levelA.getBackgroundPath();

            if (path != null) {
                // try loading
                backgroundTex = new Texture(Gdx.files.internal(path));
            }
        } catch (Exception ignored) {
        }

        // Recalculate total based on declared counts if available, otherwise array size
        try {

            // if Level returns shared total, we might double count if we add A + B.
            // Assume if A returns > 0, use it.
            totalDocuments = (levelA != null) ? levelA.getTotalDocuments() : documents.size;
        } catch (Exception ignored) {
            totalDocuments = documents.size;
        }
        ...
        levelComplete = false;
        Gdx.app.log("LevelManager2",
                "Loaded two maps. Total Docs: " + totalDocuments + " Shredder: " + (shredder != null));
    }
    ...
    // Methods to control the shared shredder
    public void setSharedShredderReady() {
        if (sharedShredder != null)
            sharedShredder.setReady();
    }

    public void setSharedShredderActive() {
        if (sharedShredder != null)
            sharedShredder.setActive();
    }

    public void setSharedShredderIdle() {
        if (sharedShredder != null)
            sharedShredder.setIdle();
    }

    public LevelShredder getSharedShredder() {
        return sharedShredder;
    }

    public void setSharedShredder(LevelShredder s) {
        this.sharedShredder = s;
    }
