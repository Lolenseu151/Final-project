...
public class JSObstacle {
    ...
    // Instances to draw (each holds a reference to the same Rectangle returned to
    // levels)
    private static final ArrayList<JSObstacle> instances = new ArrayList<>();
    // When one JS catches the player, prevent other JS instances from also entering
    // caught state
    ...
    // Vertical tolerance (in pixels) for sight checks — player center must be
    // within this
    ...
    private static void ensureFramesLoaded() {
        ...
        try {
            ...
            String base = "assets/sprites/ui/overlays/kmjs";
            for (int i = 1; i <= 4; i++) {
            ...
            // If still empty, try listing png files in known directories and load them
            // sorted by name.
            if (tmp.isEmpty()) {
                // Fallback logic removed as 'bases' was undefined
            }

            if (tmp.size() > 0)
                sharedFrames = tmp.toArray(new TextureRegion[0]);
            if (sharedFrames != null && !framesLogged) {
                Gdx.app.log("JSObstacle", "Loaded JS frames: " + sharedFrames.length);
                framesLogged = true;
            }

            // load optional caught texture: prefer a file explicitly named '5.png' else
            // pick a frame that contains '5' in name
            try {
                // try exact name first
                String p5 = "assets/sprites/ui/overlays/kmjs/5.png";
                ...
                Gdx.app.log("JSObstacle", "Loaded caught texture: " + p5);
            } catch (Exception e) {
                // if not found, scan directory for file names containing '5' (handles names
                // like '5 (3).png')
                try {
                    com.badlogic.gdx.files.FileHandle d = Gdx.files.internal("assets/sprites/ui/overlays/kmjs");
                    if (d != null && d.exists() && d.isDirectory()) {
                        ...
                        for (com.badlogic.gdx.files.FileHandle fh : files) {
                            if (fh.name().contains("5")) {
                                try {
                                    sharedCaughtTex = new Texture(fh);
                                    sharedCaughtTex.setFilter(Texture.TextureFilter.Linear,
                                            Texture.TextureFilter.Linear);
                                    Gdx.app.log("JSObstacle", "Loaded caught texture: " + fh.path());
                                    break;
                                } catch (Exception ignored) {
                                }
                            }
                ...
                if (sharedCaughtTex != null) {
                    try {
                        sharedCaughtRegion = new com.badlogic.gdx.graphics.g2d.TextureRegion(sharedCaughtTex);
                    } catch (Exception ignored) {
                        sharedCaughtRegion = null;
                    }
                }
            }
        } catch (Exception ignored) {
    ...
    public static Rectangle addTo(Array<Rectangle> obstacles, float x, float y, float w, float h) {
        ...
        instances.add(inst);
        Gdx.app.log("JSObstacle", String.format("Added JS instance at %.1f,%.1f size %.1fx%.1f (total=%d)", r.x, r.y,
                r.width, r.height, instances.size()));
        return r;
    }
    ...
    public static Rectangle addTo(Array<Rectangle> obstacles,
        ...
        instances.add(inst);
        Gdx.app.log("JSObstacle", String.format(
                "Added JS instance (configured) left=%.1f right=%.1f y=%.1f size=%.1fx%.1f speed=%.1f sight=%.1f (total=%d)",
                leftX, rightX, y, r.width, r.height, speed, sightDistance, instances.size()));
        return r;
    }

    /**
     * Returns true if the provided rectangle belongs to a registered JS visual
     * instance.
     */
     ...
     */
    public static void updateAll(float dt, Player player) {
    ...
    private void update(float dt, Player player) {
        // If we're in a caught pending state, advance its timer and trigger GAMEOVER
        // after `gameOverDelay`.
        if (caughtPending) {
            caughtTimer += dt;
        ...
        if (player != null && player.getBounds() != null) {
            ...
            boolean playerIsInFront = (facingRight && playerCenterX > jsCenterX)
                    || (!facingRight && playerCenterX < jsCenterX);

            {
                ...
                // Only catch if player is in front (sight) OR if directly overlapping while
                // also in sight.
                // This prevents the player from being caught when sneaking behind JS or on
                // other floors.
                if (inSight || overlapAllowed) {
                    ...
                    try {
                        ...
                        float jsCenterY = rect.y + rect.height * 0.5f;
                        Gdx.app.log("JSObstacle", String.format(
                                "CatchAttempt idx=%d jsX=%.1f playerX=%.1f jsY=%.1f playerY=%.1f inSight=%b overlap=%b playerIsInFront=%b",
                                idx, jsCenterX, playerCenterX, jsCenterY, playerCenterY, inSight, directOverlap,
                                playerIsInFront));
                    } catch (Exception ignored) {
                    }
                    ...
                    if (!anyCaughtActive) {
                        ...
                        try {
                            ...
                            if (appObj != null) {
                                try {
                                    java.lang.reflect.Method getHsm = appObj.getClass()
                                            .getMethod("getHoverSoundManager");
                        ...
                        // GAMEOVER will be triggered after `gameOverDelay` elapses (caughtPending
                        // timer)
    ...
    // Helper to set the screen's currentState to GAMEOVER via reflection (mirrors
    // Level2 behavior)
            ...
            if (app instanceof com.badlogic.gdx.Game) {
                ...
                if (screen != null) {
                    ...
                    try {
                        ...
                        if (sc != null) {
                            try {
                                f = sc.getDeclaredField("currentState");
                            } catch (Exception ignored) {
                            }
                        }
                    }
            ...
            while (it.hasNext()) {
                ...
                if (jo.caughtPending && sharedCaughtRegion != null) {
                    try {
                        ...
                        // If the JS is facing left, mirror the caught texture so the reaction faces the
                        // player
