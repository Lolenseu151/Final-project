...
/**
 * Player (formerly Fixer): simple character controller + animation state
 * machine.
...
public class Player {
    ...
    // Toggle to enable debug logging inside update() (set to true when diagnosing
    // input/freeze issues)
    ...
    public Player(float x, float y, Texture playerTexture) {
        bounds = new Rectangle(x, y, WIDTH, HEIGHT);

        // try load assets (several candidate names)
        standingTex = safeLoad("assets/sprites/player/standing.png", "Standing.png", "standing.png");
        jumpTex = safeLoad("assets/sprites/player/jumping.png", "Jump.png", "jumping.png");

        // load three running frames: running_1.png, running_2.png, running_3.png
        runTex1 = safeLoad("assets/sprites/player/running_1.png", "running1.png", "running1.PNG");
        runTex2 = safeLoad("assets/sprites/player/running_2.png", "running2.png", "running2.PNG");
        runTex3 = safeLoad("assets/sprites/player/running_3.png", "running3.png", "running3.PNG");

        // load dash effect sprite sheet and create animation
        dashEffectTex = safeLoad("assets/sprites/player/dashing.png", "dashing.png");
        if (dashEffectTex != null) {
            ...
            Gdx.app.log("Player", "Loaded dash effect animation with " + dashFrames.size + " frames");
        }

        if (standingTex != null)
            standingFrame = new TextureRegion(standingTex);
        if (jumpTex != null)
            jumpFrame = new TextureRegion(jumpTex);
        if (runTex1 != null)
            runFrame1 = new TextureRegion(runTex1);
        if (runTex2 != null)
            runFrame2 = new TextureRegion(runTex2);
        if (runTex3 != null)
            runFrame3 = new TextureRegion(runTex3);

        // fallback to shared texture if provided
        if (standingFrame == null && playerTexture != null)
            standingFrame = new TextureRegion(playerTexture);
        ...
        Gdx.app.log("Player", "Init frames: standing=" + (standingFrame != null) + " run=" + (runAnim != null)
                + " jump=" + (jumpFrame != null) + " dashAnim=" + (dashEffectAnim != null));
    }
    ...
    private Texture safeLoad(String... candidates) {
        for (String c : candidates) {
            try {
                ...
                if (fh.exists()) {
                    Gdx.app.log("Player", "Loading internal: " + c);
                    return new Texture(fh);
                }
                fh = Gdx.files.absolute(c);
                if (fh.exists()) {
                    Gdx.app.log("Player", "Loading absolute: " + c);
                    return new Texture(fh);
                }
            } catch (Exception e) {
                Gdx.app.log("Player", "Failed to load " + c + ": " + e.getMessage());
            }
        }
        return null;
    }
    ...
    public void update(float dt) {
        ...
        if (frozen) {
            if (DEBUG_LOG_INPUTS)
                Gdx.app.log("Player", "update skipped because frozen=true");
        ...
        if (bounds.y < -50f) {
            ...
            isOnGround = true;
            canJump = true;
            Gdx.app.log("Player", "Fell off world, respawning at y=50");
        }
        ...
        if (DEBUG_LOG_INPUTS) {
            try {
                Gdx.app.log("Player", String.format("Inputs: left=%b right=%b jumpJust=%b dashJust=%b isOnGround=%b",
                        left, right, jumpPressed, dashPressed, isOnGround));
        ...
        if (dashTimer <= 0f) {
            ...
            velocity.x = com.badlogic.gdx.math.MathUtils.clamp(velocity.x, -MAX_MOVE_SPEED * speedMult,
                    MAX_MOVE_SPEED * speedMult);
        }
        ...
        if (dashPressed && dashCooldownTimer <= 0f) {
            ...
            Gdx.app.log("Player", "DASH triggered - burst movement");
        }
        ...
        if (jumpPressed && isOnGround && canJump) {
            ...
            isOnGround = false;
            canJump = false;
            Gdx.app.log("Player", "JUMP triggered! JUMP_VY=" + JUMP_VY + " current_vy=" + velocity.y + " GRAVITY="
                    + GRAVITY + " isOnGround=" + isOnGround);
        }
        ...
        if (!isOnGround && Math.abs(oldY - bounds.y) > 5f && stateTime < 2f) {
            Gdx.app.log("Player", "Y-position: " + bounds.y + " (delta: " + (bounds.y - oldY) + ") velocity.y="
                    + velocity.y + " dt=" + dt);
        }
        ...
        if (dashTimer <= 0f) {
            if (!isOnGround) {
                ...
                // lower run threshold so running activates earlier and make idle threshold
                // small
        ...
        switch (state) {
            ...
            case RUN:
                if (runAnim != null)
                    next = runAnim.getKeyFrame(stateTime, true);
                else if (runFrame1 != null)
                    next = runFrame1;
                break;
            case DASH:
            case IDLE:
    ...
    public void reset(float x, float y) {
        ...
        if (!allowAutoReset) {
            com.badlogic.gdx.Gdx.app.log("Player", "reset() suppressed while paused/minimized");
            return;
        }
        ...
        isOnGround = true;
        canJump = true;
        Gdx.app.log("Player", "Reset to position (" + x + "," + y + ") onGround=true");
    }
