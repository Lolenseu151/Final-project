...
public class MainMenuScreen implements Screen {
    ...
    // screen-relative height (fraction of screen height) used when
    // RUN_SIZE_MODE_SCREEN_REL
    ...
    @Override
    public void show() {
        // Prefer a provided TTF (pixel font) and generate a BitmapFont at runtime using
        // FreeType
        // Build a list of candidate TTF paths to try (variable font, explicit regular,
        // and any in static/)
        List<String> candidates = new ArrayList<>();
        // Removed dead code searching for fonts/Pixelify_Sans/static which does not
        // exist

        boolean generated = false;

        // PRIORITY: try the Gradient Pexilify bitmap font first so it overrides other
        // fallbacks
        try {
            String gradFnt = "assets/fonts/Gradient/Gradient pexilify.fnt";
            String gradPng = "assets/fonts/Gradient/Gradient pexilify.png";
            ...
            if (Gdx.files.internal(gradFnt).exists() && Gdx.files.internal(gradPng).exists()) {
                ...
                String absFnt = userDir + "/" + gradFnt;
                String absPng = userDir + "/" + gradPng;
            ...
            if (fntHandle != null && pngHandle != null) {
                ...
                if (titleFontTexture != null) {
                    try {
                        titleFontTexture.dispose();
                    } catch (Exception ignored) {
                    }
                    titleFontTexture = null;
                }
        ...
        for (String ttfPath : candidates) {
            ...
            if (Gdx.files.internal(ttfPath).exists()) {
                ...
                try {
                    ...
                    Gdx.app.log("MainMenuScreen", "Failed to generate Pixelify font from TTF (" + ttfPath
        ...
        if (!generated && Gdx.files.internal("assets/fonts/Pexelify_Sans.fnt").exists()) {
            Gdx.app.log("MainMenuScreen", "Found .fnt font, loading BitmapFont");
            try {
                titleFont = new BitmapFont(Gdx.files.internal("assets/fonts/Pexelify_Sans.fnt"));
        ...
        if (!generated) {
            Gdx.app.log("MainMenuScreen",
                    "No Pixelify font assets found via internal paths; trying absolute asset paths...");

            // Try absolute paths (useful when running from Gradle where internal lookup may
            // not find assets)
            String userDir = System.getProperty("user.dir");
            for (String ttfPath : candidates) {
                // ttfPath usually doesn't have "assets/" in the old candidates list, but now we
                // cleared the list logic
                // For safety, assume ttfPath might need it if we added any back
                String absPath = userDir + "/" + ttfPath;
                ...
                if (Gdx.files.absolute(absPath).exists()) {
                    ...
                    try {
                        ...
                        Gdx.app.log("MainMenuScreen",
                                "Failed to generate font from absolute path " + absPath + ": " + e.getMessage());
                        titleFont = null;
                        foundTtfPath = null;
                    }
                }
            ...
            if (!generated) {
                Gdx.app.log("MainMenuScreen",
                        "No Pixelify font assets found; using default font with nearest filtering");
                titleFont = null;
            }
        }

        // If still not generated, try loading any .fnt present in assets/fonts (handles
        // unexpected filenames)
        if (!generated) {
            FileHandle fontsRoot = Gdx.files.internal("assets/fonts");
                ...
                for (FileHandle fh : fontsRoot.list()) {
                    if (fh.extension() != null && fh.extension().equalsIgnoreCase("fnt")) {
                        try {
                            ...
                            if (titleFont.getRegion() != null && titleFont.getRegion().getTexture() != null) {
                                titleFont.getRegion().getTexture().setFilter(TextureFilter.Nearest,
                            ...
                            Gdx.app.log("MainMenuScreen",
                                    "Failed to load .fnt at " + fh.path() + ": " + e.getMessage());
                        }
                    }
                }
            }
        }
        // Final explicit attempt: try loading the specific Pexelify_Sans.fnt via
        // internal and absolute paths
        if (!generated) {
            String explicitInternal = "assets/fonts/Pexelify_Sans.fnt";
            String userDir = System.getProperty("user.dir");
            String explicitAbsolute = userDir + "/" + explicitInternal;
            Gdx.app.log("MainMenuScreen", "Attempting explicit loads: internal(" + explicitInternal + ") and absolute("
                    + explicitAbsolute + ")");
            try {
        ...
        // Try loading the Gradient Pexilify font (.fnt + .png) from
        // assets/fonts/Gradient
        if (!generated) {
            String gradFnt = "assets/fonts/Gradient/Gradient pexilify.fnt";
            String gradPng = "assets/fonts/Gradient/Gradient pexilify.png";
            try {
                boolean fntExists = Gdx.files.internal(gradFnt).exists()
                        || Gdx.files.absolute(System.getProperty("user.dir") + "/" + gradFnt).exists();
                boolean pngExists = Gdx.files.internal(gradPng).exists()
                        || Gdx.files.absolute(System.getProperty("user.dir") + "/assets/" + gradPng).exists();
                if (fntExists && pngExists) {
                    // prefer internal if available
                    FileHandle fntHandle = Gdx.files.internal(gradFnt).exists() ? Gdx.files.internal(gradFnt)
                            : Gdx.files.absolute(System.getProperty("user.dir") + "/" + gradFnt);
                    FileHandle pngHandle = Gdx.files.internal(gradPng).exists() ? Gdx.files.internal(gradPng)
                            : Gdx.files.absolute(System.getProperty("user.dir") + "/" + gradPng);
                    titleFontTexture = new Texture(pngHandle);
                    titleFontTexture.setFilter(TextureFilter.Linear, TextureFilter.Linear);
                    titleFont = new BitmapFont(fntHandle, new TextureRegion(titleFontTexture), false);
                    generated = true;
        ...
        // If no pixel font was created, force the default font to nearest filtering so
        // scaled text appears pixelated
        if (titleFont == null && game.font != null && game.font.getRegion() != null
                && game.font.getRegion().getTexture() != null) {
            game.font.getRegion().getTexture().setFilter(TextureFilter.Nearest, TextureFilter.Nearest);
        }

        // Load button font: prefer a black Pexelify font if provided, then fall back to
        // Pexelify_Sans
        try {
            String[] preferred = new String[] {
                    "assets/fonts/black pexelify/black.fnt",
                    "assets/fonts/Pexelify_Sans.fnt"
            };
        ...
        // Load running character sprite sheet: fixed path
        try {
            FileHandle runHandle = null;
            // Updated path to point to assets/sprites/ui/main_menu/
            runHandle = Gdx.files.internal("assets/sprites/ui/main_menu/fixer run.png");

            if (runHandle != null && runHandle.exists()) {
                ...
                for (int i = 0; i < runColumns; i++)
                    frames[i] = tmp[0][i];
                runAnimation = new Animation<TextureRegion>(MENU_RUN_FRAME_DURATION, frames);
            } else {
                Gdx.app.log("MainMenuScreen", "fixer run.png not found in sprites/ui/main_menu/");
            }
        } catch (Exception e) {
        ...
        try {
            // Updated path
            FileHandle catHandle = Gdx.files.internal("assets/sprites/ui/main_menu/Catrun.png");
            if (catHandle != null && catHandle.exists()) {
                ...
                catAnimation = null;
                catTexture = null;
                Gdx.app.log("MainMenuScreen", "Catrun.png not found.");
            }
        } catch (Exception e) {
        ...
        try {
            // Updated path
            FileHandle bgHandle = Gdx.files.internal("assets/sprites/ui/main_menu/MainMenuBG.png");
            if (bgHandle.exists()) {
                backgroundTex = new Texture(bgHandle);
                backgroundTex.setFilter(TextureFilter.Linear, TextureFilter.Linear);
                Gdx.app.log("MainMenuScreen", "Loaded background: assets/sprites/ui/main_menu/MainMenuBG.png");
            } else {
                Gdx.app.log("MainMenuScreen", "MainMenuBG.png not found.");
            }
        } catch (Exception e) {
        ...
        // NEW: load bird sprite (try even-split first using birdColumns; fallback to
        // auto-detect)
        try {
            // Updated path
            FileHandle birdHandle = Gdx.files.internal("assets/sprites/ui/main_menu/birdfly.png");

            if (birdHandle != null && birdHandle.exists()) {
                ...
                try {
                        ...
                        if (frameW > 0) {
                            ...
                            if (tmp.length > 0 && tmp[0].length > 0) {
                                ...
                                for (int i = 0; i < tmp[0].length; i++) {
                                    ...
                                    boolean any = false;
                                    outer: for (int cx = Math.max(0, sx); cx < Math.min(texW, sx + sw); cx++) {
                                ...
                                if (validFrames >= Math.max(1, tmp[0].length / 2)) {
                                    ...
                                    for (TextureRegion r : fa) {
                                        if (r.getRegionWidth() > maxW)
                                            maxW = r.getRegionWidth();
                                        if (r.getRegionHeight() > maxH)
                                            maxH = r.getRegionHeight();
                                    }
                                    birdFrameW = maxW;
                                    birdFrameH = maxH;
                                    usedEvenSplit = true;
                                    Gdx.app.log("MainMenuScreen",
                                            "birdfly: used even split frames=" + fa.length + " frameW=" + frameW);
                                }
                            }
                        }
                    }
                    ...
                    if (!usedEvenSplit) {
                        ...
                        for (int x = 0; x < texW; x++)
                            occupied[x] = colCount[x] >= minOpaque;
                        ...
                        while (x < texW) {
                            ...
                            while (x < texW) {
                                ...
                                int run = 0;
                                int j = x;
                                while (j < texW && !occupied[j] && run <= separatorWidth) {
                                    run++;
                                    j++;
                                }
                                if (run > separatorWidth)
                                    break;
                                x = j;
                            }
                        ...
                        if (frames.isEmpty())
                            frames.add(new TextureRegion(birdTexture));
                        ...
                        birdFrameW = maxW;
                        birdFrameH = maxH;
                        Gdx.app.log("MainMenuScreen", "birdfly: auto-detected frames=" + fa.length + " maxW="
                                + birdFrameW + " maxH=" + birdFrameH);
                    }
                } finally {
                    if (pm != null)
                        pm.dispose();
                ...
                birdTexture = null;
                birdAnimation = null;
                Gdx.app.log("MainMenuScreen", "birdfly.png not found.");
            }
        } catch (Exception e) {
        ...
        // Removed validation to prevent startup errors for missing files.
        /*
         * try {
         * String baseDir = "assets/Start/";
         * 
         * // tutorial
         * tutorialButtonTexture = new Texture(Gdx.files.internal(baseDir + "1.png"));
         * tutorialButtonHoverTexture = new Texture(Gdx.files.internal(baseDir +
         * "2.png"));
         * // start
         * startButtonTexture = new Texture(Gdx.files.internal(baseDir + "3.png"));
         * startButtonHoverTexture = new Texture(Gdx.files.internal(baseDir + "4.png"));
         * // settings
         * settingsButtonTexture = new Texture(Gdx.files.internal(baseDir + "5.png"));
         * settingsButtonHoverTexture = new Texture(Gdx.files.internal(baseDir +
         * "6.png"));
         * 
         * // Apply smoothing filter so scaling looks good while preserving aspect
         * Texture[] texs = new Texture[] { tutorialButtonTexture,
         * tutorialButtonHoverTexture,
         * startButtonTexture, startButtonHoverTexture, settingsButtonTexture,
         * settingsButtonHoverTexture };
         * for (Texture t : texs) {
         * if (t != null)
         ...
         * 
         * Gdx.app.log("MainMenuScreen",
         * "Loaded Start button images (if present) from assets/Start/");
         * } catch (Exception e) {
    ...
    private void generateTitleFontWithSize(int size) {
        ...
        try {
            ...
            if (Gdx.files.internal(foundTtfPath).exists()) {
                ...
                if (Gdx.files.absolute(alt).exists())
                    ttfHandle = Gdx.files.absolute(alt);
            }
            if (ttfHandle == null) {
            ...
            generator.dispose();
            Gdx.app.log("MainMenuScreen",
                    "Generated BitmapFont from TTF (size=" + parameter.size + ") using " + foundTtfPath);
        } catch (Exception e) {
    ...
    private void drawMenuOptionBox(float centerX, float y, MenuOption option) {
        // If the START_GAME/TUTORIAL/SETTINGS option has a custom image, skip drawing
        ...
        shapeRenderer.setColor(
                isSelected ? new Color(0f, 0.6f, 0.6f, 0.95f) : new Color(217 / 255f, 217 / 255f, 217 / 255f, 1f));
        // center large rect
    ...
    private void drawMenuOptionText(String text, float x, float y, MenuOption option) {
        ...
        boolean isSelected = (hoveredOption == option);
        // Choose the button font if available, otherwise fall back to the game's
        // default font
        ...
        if (option == MenuOption.TUTORIAL && tutorialButtonTexture != null) {
            Texture tex = isSelected && tutorialButtonHoverTexture != null ? tutorialButtonHoverTexture
        ...
        if (option == MenuOption.SETTINGS && settingsButtonTexture != null) {
            Texture tex = isSelected && settingsButtonHoverTexture != null ? settingsButtonHoverTexture
        ...
        // apply requested scale while measuring and drawing, then restore previous
        // scale
        ...
        // inside a box whose center is at `y`, shift baseline up by half the layout
        // height.
        float textY = y + (layout.height / 2f);

        // Indicate selection by configurable color (no arrow)
        // Use the SpriteBatch color to force a consistent tint regardless of font
        // internals
        Color prevBatch = game.batch.getColor().cpy();
        try {
            if (isSelected)
                game.batch.setColor(BUTTON_TEXT_COLOR_SELECTED);
            else
                game.batch.setColor(BUTTON_TEXT_COLOR);
    ...
    private void drawMenu() {
        ...
        // Draw running character animation between the title and the buttons, scaled
        // with a small bounce
        if (runAnimation != null) {
            // compute animation time (we pause advancing the runner's animation while in
            // the restart delay)
            if (!runPaused)
                runAnimTime += Gdx.graphics.getDeltaTime();
            ...
            if (runPaused) {
                ...
                // bounce and vertical placement as before (bounce uses menuTime so it's synced
                // to animation)
        ...
        // NEW: bird drawing (top of screen) use current frame's bounds and explicit
        // target width
        if (birdTexture != null) {
            ...
            float frameW = currentFrame != null ? currentFrame.getRegionWidth()
                    : (birdFrameW > 0 ? birdFrameW : birdTexture.getWidth());
            float frameH = currentFrame != null ? currentFrame.getRegionHeight()
                    : (birdFrameH > 0 ? birdFrameH : birdTexture.getHeight());

            // Option B: explicit on-screen width (preferred to avoid including neighbor
            // frames)
            ...
            // leftPercent/rightPercent define the horizontal start/end of the line (0..1 of
            // screen width).
            // lineYPercent defines vertical position (0..1 from bottom); increase to move
            // bird closer to top.
            ...
            // offset slightly down so bird sits on/under the line depending on sprite
            // origin
