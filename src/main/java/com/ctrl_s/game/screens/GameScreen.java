...
import com.badlogic.gdx.graphics.g2d.BitmapFont; // import check
...
import com.ctrl_s.game.levels.levels.Level1;
import com.ctrl_s.game.levels.levels.Level2;
import com.ctrl_s.game.levels.levels.Level3;

import com.ctrl_s.game.levels.levels.Level4;
import com.ctrl_s.game.levels.levels.Level5;

import com.ctrl_s.game.levels.LevelTutorial;
import com.ctrl_s.game.levels.LevelManager;
import com.ctrl_s.game.levels.LevelManager2;
...
import com.ctrl_s.game.utils.LevelMusicManager;
import com.ctrl_s.game.utils.BackgroundMusicManager;

/**
 * GameScreen with Level progression (1-5), level select, and completion
 * notifications
 */
public class GameScreen implements Screen {
    ...
    private Texture btnRestartTex; // assets/sprites/ui/buttons/restart.png
    private Texture btnResumeTex; // assets/sprites/ui/buttons/resume.png
    private Texture btnMenuTex; // assets/sprites/ui/buttons/menu.png
    private boolean pauseOverlayVisible = false;
    private float docIconY;
    // Previous hover state map (play once on enter). Hover sound provided
    // centrally.
    ...
    // Prevent the same mouse click that opened the overlay from immediately closing
    // it
    ...
    // Record the event stats to display on overlay (docs collected and time at
    // event)
    private int lastEventDocs = 0;
    private float lastEventTime = 0f;
    private boolean lastEventRecorded = false;
    // --- Talking overlay tunables (edit these PUBLIC static values to reposition
    // the overlay text) ---
    // Example: change these values at the top of this file to move the overlay
    // text.
    ...
    // Margin from screen edges (in pixels) and vertical gap above the Continue
    // button
    public static float TALKING_TEXT_MARGIN = 30f;
    public static float TALKING_TEXT_BUTTON_GAP = 8f;
    // --- Continue button tunables ---
    // If BUTTON_X/Y are >= 0 they will be used as absolute screen coordinates
    // (pixels).
    // Otherwise the button is positioned relative to the text box (default
    // behavior).
    ...
    // Vertical gap (pixels) between the two stat lines shown on overlays (documents
    // / time)
    ...
    // Per-button scale multipliers for the Win overlay (allows shrinking specific
    // buttons)
    ...
    // Optional absolute Y for the talking text box. If >=0, used as pixel Y
    // coordinate.
    // If left negative (default), the text box Y is computed independently of the
    // button.
    ...
    // Tutorial document positions have been moved to
    // `LevelTutorial.TUTORIAL_DOC_POSITIONS`.
     ...
     */
    public GameScreen(GameApplication game, int level) {
        this.game = game;
    ...
    @Override
    public void show() {
        // Avoid re-initializing everything if show() is called again (e.g. on
        // minimize/restore)
        if (initialized) {
        ...
        // Use LevelManager2 for multi-map levels (3 and 5) to enable map transitions
        // with shared progress
        if (currentLevel == 3 || currentLevel == 5) {
            levelManager = null;
            levelManager2 = new LevelManager2();
            // Load only the first map initially - the level will handle transitioning to
            // the second map
            if (level != null) {
                ...
                try {
                    ...
                    levelManager2.setDocumentCollectedListener(new LevelManager2.DocumentCollectedListener() {
                        ...
                        public void onDocumentCollected(int collected, int total) {
                            try {
                                if (game != null && game.getHoverSoundManager() != null)
                                    game.getHoverSoundManager().playDocument();
            ...
            if (level != null) {
                ...
                if (currentLevel == 1 && level instanceof Level1) {
                    Gdx.app.log("GameScreen",
                            "Level1 loaded - restoring intro state: level1IntroShown=" + level1IntroShown);
                    ((Level1) level).setIntroShown(level1IntroShown);
                }
                // Start playing background music for the level using global manager
                String musicPath = level.getMusicPath();
                if (musicPath != null && !musicPath.isEmpty()) {
                ...
                try {
                    Class<?> listenerClass = Class.forName("com.ctrl_s.game.levels.LevelManager$LevelCompleteListener");
                    java.lang.reflect.Method setMethod = levelManager.getClass().getMethod("setLevelCompleteListener",
                            listenerClass);
                ...
                try {
                    try {
                        Class<?> shredClass = Class.forName("com.ctrl_s.game.levels.LevelManager$ShredStartListener");
                        java.lang.reflect.Method setShredMethod = levelManager.getClass().getMethod(
                                "setShredStartListener",
                                shredClass);
                                            ...
                                            try {
                                                Gdx.app.postRunnable(new Runnable() {
                                                    ...
                                                    public void run() {
                                                        try {
                                                            if (game != null && game.getHoverSoundManager() != null)
                                                                game.getHoverSoundManager().playShred();
                                                            try {
        ...
        // Ensure raw keyboard input is delivered to Player (prevents UI stage or other
        // processors
        // from blocking keys). This doesn't change game logic using it only sets the
        // input target.
        ...
        if (player != null) {
            ...
            try {
                if (currentLevel == 1) {
                    ...
                    player.getBounds().setSize(64f, 64f);
                    Gdx.app.log("GameScreen", "Applied Level1-specific player size: 64x64");
                }
            } catch (Exception ignored) {
            }
            // If we are here because the app was paused (minimized), avoid calling reset()
            // which moves the player to a spawn. Instead restore the saved position if
            // available.
            if (wasPaused && !Float.isNaN(savedX)) {
                try {
                    Gdx.app.log("GameScreen", "Restoring position in show(): savedX=" + savedX + ", savedY=" + savedY
                            + ", onGround=" + savedOnGround);
                    player.getBounds().setPosition(savedX, savedY);
                    player.setOnGround(savedOnGround);
                    com.badlogic.gdx.math.Vector2 vel = player.getVelocity();
                    if (vel != null)
                        vel.set(savedVelX, savedVelY);
                    Gdx.app.log("GameScreen",
                            "After restore in show() - actual position: x=" + player.getBounds().x + ", y="
                                    + player.getBounds().y);
                } catch (Exception ignored) {
                }
                ...
                if (currentLevel == 0) {
                    ...
                    // Spawn VERY HIGH to ensure character is visible (on third floor platform)
                    float spawnX = 500f; // Middle of third floor platform
                    float spawnY = 440f; // Third floor platform (y=420+20=440)

                    player.reset(spawnX, spawnY);
                    // Force position directly in case reset is suppressed
                    player.getBounds().setPosition(spawnX, spawnY);
                    player.getVelocity().set(0, 0);
                    player.setOnGround(true);
                    Gdx.app.log("GameScreen",
                            "Level 3: FORCED HIGH spawn at x=" + spawnX + ", y=" + spawnY
                                    + " (third floor - HIGHLY VISIBLE)");
                    ...
                    player.setOnGround(true);
                    Gdx.app.log("GameScreen",
                            "Level 4: FORCED HIGH spawn at x=" + spawnX + ", y=" + spawnY
                                    + " (third floor - HIGHLY VISIBLE)");
                } else if (currentLevel == 5 && level instanceof Level5) {
                    try {
                        float[] sp = ((Level5) level).getEntranceSpawn();
                        if (sp != null && sp.length >= 2)
                            player.reset(sp[0], sp[1]);
                        else
                            player.reset(640, 580);
                    } catch (Exception e) {
                        player.reset(640, 580);
                    }
                } else {
        ...
        // mark as initialized so future show() calls (from minimize/restore) do not
        // reload/reset
        initialized = true;

        // Load overlay arrow texture (optional)
        try {
            overlayArrowTex = new com.badlogic.gdx.graphics.Texture(
                    Gdx.files.internal("assets/sprites/ui/overlays/arrow.png"));
        } catch (Exception e) {
            overlayArrowTex = null;
        }
        // Load the full-screen overlay image (shown after OK)
        try {
            overlayFullTex = new com.badlogic.gdx.graphics.Texture(
                    Gdx.files.internal("assets/sprites/ui/overlays/full_overlay.png"));
        } catch (Exception e) {
            overlayFullTex = null;
        }
        // Load win / gameover overlays
        try {
            overlayWinTex = new com.badlogic.gdx.graphics.Texture(
                    Gdx.files.internal("assets/sprites/ui/overlays/win_overlay.png"));
        } catch (Exception e) {
            overlayWinTex = null;
        }
        try {
            overlayGameOverTex = new com.badlogic.gdx.graphics.Texture(
                    Gdx.files.internal("assets/sprites/ui/overlays/gameover.png"));
        } catch (Exception e) {
            overlayGameOverTex = null;
        }
        // Load the tutorial talking overlay (shown when first document collected)
        try {
            overlayTalkingTex = new com.badlogic.gdx.graphics.Texture(
                    Gdx.files.internal("assets/sprites/ui/overlays/talking_overlay.png"));
        } catch (Exception e) {
            try {
                overlayTalkingTex = new com.badlogic.gdx.graphics.Texture(
                        Gdx.files.internal("assets/sprites/ui/overlays/talking_overlay_alt.png"));
            } catch (Exception ex) {
                overlayTalkingTex = null;
            }
        }
        // Load the small white font for overlay text (optional)
        try {
            overlayTalkingFont = new com.badlogic.gdx.graphics.g2d.BitmapFont(
                    Gdx.files.internal("assets/fonts/Small_white.fnt"),
                    Gdx.files.internal("assets/fonts/Small_white.png"),
                    false);
            Gdx.app.log("GameScreen", "Loaded Small_white font from assets/fonts/");
        } catch (Exception e) {
        ...
        try {
            ...
            statFont = null;
            Gdx.app.log("GameScreen", "Stat font not found; using default font");
        }

        // Load a smaller font for the document counter so it doesn't share the large
        // uiFont scale
        try {
        ...
        // (Removed) timer icon - we now render the audit timer centered using the audit
        // background image

        try {
            documentSheetTexture = new Texture(Gdx.files.internal("assets/sprites/items/documents.png"));
            // Split into 2 columns x 3 rows
            int cols = 2, rows = 3;
            int frameW = documentSheetTexture.getWidth() / cols;
            int frameH = documentSheetTexture.getHeight() / rows;
            com.badlogic.gdx.graphics.g2d.TextureRegion[][] tmp = com.badlogic.gdx.graphics.g2d.TextureRegion
                    .split(documentSheetTexture, frameW, frameH);
            ...
            Gdx.app.log("GameScreen", "Loaded document spritesheet (2x3): assets/sprites/items/documents.png");
        } catch (Exception e) {
        ...
        try {
            auditBgTexture = new Texture(Gdx.files.internal("assets/sprites/ui/hud/audit_time_bg.png"));
            Gdx.app.log("GameScreen", "Loaded audit timer background: assets/sprites/ui/hud/audit_time_bg.png");
        } catch (Exception ex) {
            auditBgTexture = null;
        ...
        try {
            pauseButtonTexture = new Texture(Gdx.files.internal("assets/sprites/ui/overlays/pause_icon.png"));
        } catch (Exception ex) {
            pauseButtonTexture = null;
        }
        // Hover sound provided centrally via HoverSoundManager on MyGdxGame
        try {
            overlayPauseTex = new Texture(Gdx.files.internal("assets/sprites/ui/overlays/pause_overlay.png"));
        } catch (Exception ex) {
            overlayPauseTex = null;
        }

        // Load pause overlay buttons: restart, resume, menu
        try {
            btnRestartTex = new Texture(Gdx.files.internal("assets/sprites/ui/buttons/restart.png"));
        } catch (Exception ex) {
            btnRestartTex = null;
        }
        try {
            btnResumeTex = new Texture(Gdx.files.internal("assets/sprites/ui/buttons/resume.png"));
        } catch (Exception ex) {
            btnResumeTex = null;
        }
        try {
            btnMenuTex = new Texture(Gdx.files.internal("assets/sprites/ui/buttons/menu.png"));
        } catch (Exception ex) {
            btnMenuTex = null;
        }
        ...
        try {
            clockTexture = new Texture(Gdx.files.internal("assets/sprites/ui/overlays/clock.png"));
        } catch (Exception ex) {
            clockTexture = null;
        }
    ...
    @Override
    public void render(float delta) { // If the win overlay is visible we prefer not to introduce an opaque
        ...
        // If level complete, still render the game underneath and then draw overlay on
        // top
        if (showLevelComplete) {
            ...
            try {
                ...
                Gdx.app.log("GameScreen",
                        "showLevelComplete triggered - winOverlayVisible=" + winOverlayVisible + ", overlayWinTex="
                                + (overlayWinTex != null) + ", levelBgPresent=" + hasBg + ", currentLevel="
                                + currentLevel);
            } catch (Exception ignored) {
            }
    ...
    private void handleInput() {
        ...
        if (currentState == GameState.GAMEOVER && !lastEventRecorded) {
            try {
                ...
                try {
                    if (game != null && game.getHoverSoundManager() != null)
                        game.getHoverSoundManager().playGameOver();
                } catch (Exception ignored) {
                }
                try {
                    if (musicManager != null)
    ...
    private void update(float deltaTime) {
        ...
        // Update floating animation timer - removed unused
        ...
        try {
            if (levelManager != null) {
                ...
                if (cur != null) {
                    ...
                    try {
                        if (cur instanceof BackgroundedLevel)
                            blocking = ((BackgroundedLevel) cur).isOverlayBlocking();
                    } catch (Exception ignored) {
                    }
                    Gdx.app.log("GameScreenDebug",
                            "ActiveLevel=" + lvlName + " overlayFullVisible=" + overlayFullVisible
                                    + " overlaySuppressNextClick=" + overlaySuppressNextClick + " blocking=" + blocking
                    ...
                    try {
                        if (cur instanceof LevelTutorial) {
                            ...
                            Gdx.app.log("GameScreenDebug",
                                    "LevelTutorial.showOverlay=" + lt.isShowOverlay() + " finalOverlayVisible="
                                            + lt.isFinalOverlayVisible() + " finalConsumed="
                                            + lt.isFinalOverlayConsumed());
                        }
                    } catch (Exception ignored) {
                    }
                }
        ...
        try {
            ...
            if (levelManager != null) {
                ...
                try {
                    ...
                    Level b = levelManager2.getCurrentLevelB();
                    Level a = levelManager2.getCurrentLevelA();
                    activeLevel = (b != null) ? b : a;
                } catch (Exception ignored) {
                }
            }
            if (activeLevel instanceof BackgroundedLevel) {
                BackgroundedLevel bl = (BackgroundedLevel) activeLevel;
                if (bl.isOverlayBlocking()) {
                    ...
                    try {
                        ILevelManager lm = (levelManager != null) ? (ILevelManager) levelManager
                                : (ILevelManager) levelManager2;
                        // Call updateBackground directly so the level can handle clicks/keys
                        bl.updateBackground(deltaTime, lm, null, null, player);
                    } catch (Exception ignored) {
                    }
                    return;
                }
        ...
        if (levelManager != null)
            timePenalty = levelManager.update(deltaTime, player);
        else if (levelManager2 != null)
            timePenalty = levelManager2.update(deltaTime, player);
        ...
        if (docsLabel != null) {
            if (levelManager != null) {
                docsLabel.setText(
                        "Documents: " + levelManager.getDocumentsCollected() + "/" + levelManager.getTotalDocuments());
            } else if (levelManager2 != null) {
                docsLabel.setText("Documents: " + levelManager2.getDocumentsCollected() + "/"
                        + levelManager2.getTotalDocuments());
            }
        }
        if (timeLabel != null) {
        ...
        if (currentLevel == 1 && levelManager != null) {
            try {
                ...
                if (level instanceof Level1) {
                    Level1 l1 = (Level1) level;
                    // Use reflection to check showIntro field
                    try {
                        java.lang.reflect.Field f = Level1.class.getDeclaredField("showIntro");
                        f.setAccessible(true);
    ...
    private void renderGame() {
        ...
        if (game != null && game.batch != null) {
            if (player != null && player.isDashing() && shapeRenderer != null) {
        ...
        // Draw the talking overlay if the tutorial level requested it (first doc
        // collected)
    ...
    private void drawPauseOverlayIfActive() {
        ...
        if (overlayPauseTex != null) {
            // Draw the overlay full-screen (the asset likely contains the darkened
            // background)
        ...
        // Button sizes (use texture native size but fit to max while preserving aspect
        // ratio)
        // Increased max to allow larger buttons; upscaling allowed so small assets can
        // be enlarged.
        float btnMaxW = 300f;
        float btnMaxH = 112f;
        ...
        // Compute drawn sizes and positions, centering scaled images on their original
        // centers
        ...
        // Draw restart (left)
        if (btnRestartTex != null)
            game.batch.draw(btnRestartTex, drawRX, btnY - (drawRH - rH) * 0.5f, drawRW, drawRH);
        // Draw resume (middle)
        if (btnResumeTex != null)
            game.batch.draw(btnResumeTex, drawSX, btnY - (drawSH - sH) * 0.5f, drawSW, drawSH);
        // Draw menu (right)
        if (btnMenuTex != null)
            game.batch.draw(btnMenuTex, drawMX, btnY - (drawMH - mH) * 0.5f, drawMW, drawMH);
        ...
        if (Gdx.input.isButtonJustPressed(com.badlogic.gdx.Input.Buttons.LEFT)) {
            ...
            if (overlaySuppressNextClick && overlaySuppressTimer > 0f) {
                boolean insideRestart = (mxIn >= drawRX && mxIn <= drawRX + drawRW
                        && myIn >= btnY - (drawRH - rH) * 0.5f && myIn <= btnY - (drawRH - rH) * 0.5f + drawRH);
                boolean insideResume = (mxIn >= drawSX && mxIn <= drawSX + drawSW && myIn >= btnY - (drawSH - sH) * 0.5f
                        && myIn <= btnY - (drawSH - sH) * 0.5f + drawSH);
                boolean insideMenu = (mxIn >= drawMX && mxIn <= drawMX + drawMW && myIn >= btnY - (drawMH - mH) * 0.5f
                        && myIn <= btnY - (drawMH - mH) * 0.5f + drawMH);
            ...
            if (mxIn >= drawRX && mxIn <= drawRX + drawRW && myIn >= btnY - (drawRH - rH) * 0.5f
                    && myIn <= btnY - (drawRH - rH) * 0.5f + drawRH) {
            ...
            // Resume
            if (mxIn >= drawSX && mxIn <= drawSX + drawSW && myIn >= btnY - (drawSH - sH) * 0.5f
                    && myIn <= btnY - (drawSH - sH) * 0.5f + drawSH) {
                pauseOverlayVisible = false;
                currentState = GameState.RUNNING;
                // Resume the music
            ...
            // Menu
            if (mxIn >= drawMX && mxIn <= drawMX + drawMW && myIn >= btnY - (drawMH - mH) * 0.5f
                    && myIn <= btnY - (drawMH - mH) * 0.5f + drawMH) {
                try {
    ...
    private void drawTutorialOverlay() {
        if (shapeRenderer == null || game == null || game.batch == null || game.font == null)
            return;
        ...
        com.badlogic.gdx.graphics.g2d.GlyphLayout glBtn = new com.badlogic.gdx.graphics.g2d.GlyphLayout(game.font,
                "OK");
        game.batch.begin();
    ...
    private void drawFloatingUI(com.badlogic.gdx.graphics.g2d.SpriteBatch batch) {
        ...
        // Time drawing will be positioned beside the document icon/text on the left
        // side.
        ...
        boolean pauseHover = (mouseX >= pauseX && mouseX <= pauseX + pauseSize && mouseY >= pauseY
        ...
        if (Gdx.input.isButtonJustPressed(com.badlogic.gdx.Input.Buttons.LEFT)) {
            float mx = mouseX;
            float my = mouseY;
            if (mx >= pauseDrawX && mx <= pauseDrawX + pauseDrawSize && my >= pauseDrawY
                    && my <= pauseDrawY + pauseDrawSize) {
        ...
        // Keep the document count text at the original visual baseline (was at
        // height-110f)
        ...
        // With a 2x3 sheet, this corresponds to zero-based r=2, c=1 -> index = 2*2 + 1
        // = 5.
        ...
        if (!docText.isEmpty()) {
            // Draw the text at the fixed baseline so it does not move when the icon Y
            // changes
        ...
        // Prepare time glyph (use dedicated timeFont if available, otherwise fall back
        // to uiFont)
        com.badlogic.gdx.graphics.g2d.BitmapFont timeFontToUse = (timeFont != null) ? timeFont : uiFont;
        com.badlogic.gdx.graphics.g2d.GlyphLayout glTime = new com.badlogic.gdx.graphics.g2d.GlyphLayout(timeFontToUse,
                timeText);
        float padding = 9f;
        // Determine desired clock height to match font height (so the icon aligns with
        // text)
    ...
    // If the full-screen overlay is active, draw it on top of everything and allow
    // dismissal
    private void drawFullOverlayIfActive() {
        if (!overlayFullVisible)
            return;
        if (game == null || game.batch == null)
            return;
        ...
        // Dismiss on ESC, Enter, Space, or mouse click, but ignore the click that
        // opened the overlay
        try {
            // If a tutorial Level is active, obtain it so we can clear its overlay flag
            // when dismissing
            LevelTutorial lt = null;
    ...
    // Draw the tutorial talking overlay (triggered when first document is
    // collected)
    private void drawTalkingOverlayIfActive() {
        // Prioritize the talking overlay only when current level is tutorial and that
        // level wants it
        if (levelManager == null)
            return;
        try {
            ...
            if (cur instanceof LevelTutorial) {
                ...
                // Compute button position: prefer explicit tunables if set, otherwise place
                // relative to edges
                ...
                com.badlogic.gdx.graphics.g2d.BitmapFont fontToUse = (overlayTalkingFont != null) ? overlayTalkingFont
                        : game.font;
                ...
                String fullText;
                if (talkingStage == 0)
                    fullText = TALKING_OVERLAY_FULL_TEXT;
                else if (talkingStage == 1)
                    fullText = TALKING_OVERLAY_SECOND_TEXT;
                else
                    fullText = TALKING_OVERLAY_THIRD_TEXT;
                ...
                if (game.batch != null && fontToUse != null) {
                    ...
                    gl.setText(fontToUse, visibleText, com.badlogic.gdx.graphics.Color.WHITE, textBoxW - textPad * 2f,
                ...
                if (game.batch != null) {
                    com.badlogic.gdx.graphics.g2d.BitmapFont btnFont = (overlayTalkingFont != null) ? overlayTalkingFont
                            : game.font;
                    com.badlogic.gdx.graphics.g2d.GlyphLayout glBtn = new com.badlogic.gdx.graphics.g2d.GlyphLayout(
                            btnFont, "Continue");
                    // Vertically center the label inside the button using the glyph height as
                    // reference.
                    ...
                    if (hovered && TALKING_BUTTON_HOVER_UNDERLINE && shapeRenderer != null) {
                        // Draw underline BELOW the bottom of the laid-out glyphs so it doesn't overlap
                        // characters.
                        ...
                        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
                        shapeRenderer.setColor(TALKING_BUTTON_HOVER_UNDERLINE_R, TALKING_BUTTON_HOVER_UNDERLINE_G,
                                TALKING_BUTTON_HOVER_UNDERLINE_B, TALKING_BUTTON_HOVER_UNDERLINE_A);
                    ...
                    if (Gdx.input.isButtonJustPressed(com.badlogic.gdx.Input.Buttons.LEFT)) {
                        if (mx >= btnX && mx <= btnX + btnW && my >= btnY && my <= btnY + btnH) {
                            ...
                            if (talkingStage == 0) {
                                ...
                                try {
                                    if (levelManager != null) {
                                        if (LevelTutorial.TUTORIAL_DOC_POSITIONS != null
                                                && LevelTutorial.TUTORIAL_DOC_POSITIONS.length > 0) {
                                            ...
                                            try {
                                                java.lang.reflect.Method m = LevelManager.class
                                                        .getMethod("addDocumentsAtPositions", float[][].class);
                                                m.invoke(levelManager, (Object) LevelTutorial.TUTORIAL_DOC_POSITIONS);
                                            } catch (NoSuchMethodException nsme) {
                                                // Method not present: fallback to adding documents without explicit
                                                // positions
                                                ...
                                                try {
                                                    levelManager
                                                            .addDocuments(LevelTutorial.TUTORIAL_DOC_POSITIONS.length);
    ...
    private void drawText() {
        if (game == null || game.batch == null || game.font == null)
            return;
    ...
    private void drawDashCooldownIndicator() {
        ...
        float maxCooldown = 10.0f; // Match DASH_COOLDOWN from Player/Fixer (10 seconds)
        float barWidth = 120f;
        float barHeight = 16f;

        // Position to the right of the clock/timer area at top-right
        // The clock/timer is positioned at the right, so position the dash indicator
        // below it
    ...
    private void drawGameOverOverlay() {
        if (shapeRenderer == null || game == null || game.batch == null)
            return;
        ...
        float w = Gdx.graphics.getWidth();
        float h = Gdx.graphics.getHeight();
        // Compute button and stat layout similar to win overlay but only two buttons:
        // Restart and Menu
        ...
        try {
            com.badlogic.gdx.graphics.g2d.GlyphLayout g1 = new com.badlogic.gdx.graphics.g2d.GlyphLayout(fontToUse,
                    docsStr);
            com.badlogic.gdx.graphics.g2d.GlyphLayout g2 = new com.badlogic.gdx.graphics.g2d.GlyphLayout(fontToUse,
                    timeStr);
            float cx = w * 0.5f;
        ...
        try {
            if (Gdx.input.isButtonJustPressed(com.badlogic.gdx.Input.Buttons.LEFT)) {
                ...
                if (overlaySuppressNextClick) {
                    boolean insideRestart = (mxIn >= drawRX && mxIn <= drawRX + drawRW
                            && myIn >= btnY - (drawRH - rH) * 0.5f && myIn <= btnY - (drawRH - rH) * 0.5f + drawRH);
                    boolean insideMenu = (mxIn >= drawMX && mxIn <= drawMX + drawMW
                            && myIn >= btnY - (drawMH - mH) * 0.5f && myIn <= btnY - (drawMH - mH) * 0.5f + drawMH);
                    overlaySuppressNextClick = false;
    ...
    /**
     * Draw the Win overlay on top of the already-rendered game (DO NOT clear the
     * screen).
     */
    private void drawWinOverlay() {
        ...
        // Draw hint
        try {
            com.badlogic.gdx.graphics.g2d.GlyphLayout hintGL = new com.badlogic.gdx.graphics.g2d.GlyphLayout(game.font,
        ...
        try {
            if (Gdx.input.isButtonJustPressed(com.badlogic.gdx.Input.Buttons.LEFT)) {
                ...
                if (overlaySuppressNextClick) {
                    boolean insideRestart = (mxIn >= drawRX && mxIn <= drawRX + drawRW
                            && myIn >= btnY - (drawRH - rH) * 0.5f && myIn <= btnY - (drawRH - rH) * 0.5f + drawRH);
                    boolean insideResume = (mxIn >= drawSX && mxIn <= drawSX + drawSW
                            && myIn >= btnY - (drawSH - sH) * 0.5f && myIn <= btnY - (drawSH - sH) * 0.5f + drawSH);
                    boolean insideMenu = (mxIn >= drawMX && mxIn <= drawMX + drawMW
                            && myIn >= btnY - (drawMH - mH) * 0.5f && myIn <= btnY - (drawMH - mH) * 0.5f + drawMH);
                    overlaySuppressNextClick = false;
                    if (!insideRestart && !insideResume && !insideMenu) {
    ...
    private void levelComplete() {
        ...
        try {
            if (game != null && game.getHoverSoundManager() != null)
                game.getHoverSoundManager().playWin();
        } catch (Exception ignored) {
        }
        BackgroundMusicManager.getInstance().stopMusic();
        ...
        lastEventRecorded = true;
        // Show the blocking win overlay and suppress the click that triggered
        // completion
        winOverlayVisible = true;
        overlaySuppressNextClick = true;
    ...
    public void pause() {
        ...
        if (player != null) {
            ...
            savedX = b.x;
            savedY = b.y;
            savedOnGround = player.isOnGround();
            try {
                com.badlogic.gdx.math.Vector2 vel = player.getVelocity();
                if (vel != null) {
                    savedVelX = vel.x;
            ...
            Gdx.app.log("GameScreen", "Saved state: pos=(" + savedX + "," + savedY + "), vel=(" + savedVelX + ","
                    + savedVelY + "), onGround=" + savedOnGround);
        }
    ...
    public void resume() {
        ...
        // restore exact saved position/velocity so character remains where the player
        // left it
        if (player != null && !Float.isNaN(savedX)) {
            try {
                Gdx.app.log("GameScreen", "Restoring position in resume(): savedX=" + savedX + ", savedY=" + savedY
                        + ", onGround=" + savedOnGround);
                player.getBounds().setPosition(savedX, savedY);
                player.setOnGround(savedOnGround);
                com.badlogic.gdx.math.Vector2 vel = player.getVelocity();
                if (vel != null)
                    vel.set(savedVelX, savedVelY);
                Gdx.app.log("GameScreen",
                        "After restore - actual position: x=" + player.getBounds().x + ", y=" + player.getBounds().y);
            } catch (Exception ignored) {
            }
        }
        currentState = GameState.RUNNING;
        ...
        // leave input processor null so UI won't accidentally receive input on
        // immediate restore.
    ...
    public void dispose() {
        ...
        if (player != null)
            player.dispose();
        if (overlayArrowTex != null)
            overlayArrowTex.dispose();
        if (overlayFullTex != null)
            overlayFullTex.dispose();
        if (overlayTalkingTex != null)
            overlayTalkingTex.dispose();
        if (overlayTalkingFont != null)
            overlayTalkingFont.dispose();
        if (auditBgTexture != null)
            auditBgTexture.dispose();
        if (clockTexture != null)
            clockTexture.dispose();
        if (documentSheetTexture != null)
            documentSheetTexture.dispose();
        if (pauseButtonTexture != null)
            pauseButtonTexture.dispose();
        if (overlayPauseTex != null)
            overlayPauseTex.dispose();
        if (btnRestartTex != null)
            btnRestartTex.dispose();
        if (btnResumeTex != null)
            btnResumeTex.dispose();
        if (btnMenuTex != null)
            btnMenuTex.dispose();
        if (docFont != null)
            docFont.dispose();
        ...
        if (statFont != null)
            statFont.dispose();
        // hoverSound is managed centrally by HoverSoundManager on GameApplication
    }
